...
my($version) = "QC-version 10/17/2022 11:20am by Mike Schmidt, mschmidt\@med.miami.edu\n";
if($#ARGV != 0){
	print "$version";
...
my($output_dir) = $parameters{output_dir};
my(@outnames);
my($outname) = "";
my($line);
my(@lst);
...
if($thread_cnt > 0){
	...
	$vcf = reverse($vcf);
	for($i = 0; $i < $thread_cnt; $i++){
		$ret_code = $threads[$i]->join();
		print stderr "Thread $i returned\n";
		$temp = "\t$scratch_dir" . "flagged_" . "thread_" . "$i" . "_" . $vcf;
		$cat_string_vcf .= $temp;
	}
	$outname = $output_dir . "flagged_" . $vcf;
print "cat $cat_string_vcf > $outname\n";
	`cat $cat_string_vcf > $outname`;
	@lst = split(/\s+/, $cat_string_vcf);
	foreach $temp (@lst){
		unlink($temp);
	}
	for($i = 0; $i < $thread_cnt; $i++){
		$j = 0;
		$temp = $scratch_dir . "reunion_" . "$i" . "_snv.txt";
		open(inf,"<$temp") or die "Can not open $temp\n";
		if($i == 0){
			while(chomp($line = <inf>)){
				$cat_string_snv[$j] = $line;
				$outnames[$j] = $line;
				$j++;
			}
		}
...
# All threads returned at this point. Now deal with the individual summaries.
my(@keep);
my(@real_sex);
my(@indivmaster);
my(@lst);
...
my($nl);
my($outfile);
open(inf,"<$scratch_dir/sex.txt");
chomp($line = <inf>);
@real_sex = split(/\s+/, $line);
close(inf);
open(inf,"<$scratch_dir/keep.txt");
chomp($line = <inf>);
@keep = split(/\s+/, $line);
close(inf);
#unlink("keep.txt");
...
my($infile);
for($i = 0; $i < $thread_cnt; $i++){
	$infile = $scratch_dir . "indiv_" . "$i" . "_.txt";
	open(inf,"<$infile") or die "Can not open $infile\n";
	$j = 0;
	while(chomp($line = <inf>)){
		@lst = split(/\s+/, $line);
		$indivmaster[$j][0] = $lst[0];
		for($k = 1; $k < @lst; $k++){
			$indivmaster[$j][$k] += $lst[$k];
		}
		$j++;
	}
	close(inf);
...
print_indiv_master(\%parameters, \@indivmaster, \@keep, \@real_sex);
cleanup(\%parameters);
...
#	open(dbg,">debug.txt");
	...
	my($scratch_dir) = $parameters{scratch_dir};									# to write temp files
	my($out_dir) = $parameters{output_dir};											# where output goes						
	...
	my($iter);
	my(@buffer);																	# to read multiple lines at a time and store them
	my($buffer_index) = 0;															# index into @buffer
	my($buffer_size) = 0;															# number of un-processed items in @buffer
	my($buffer_max) = 10;															# buffer max size
	$start_bp += 0;
	$end_bp += 0;
...
=cut
	...
	if($isx){
		...
		undef($chr23);
#		if($usegz){
#			open($chr23, "| gzip -c >> chr23.vcf.gz");
#		}
#		else{
		$temp = $th_name . "_chr23.vcf";
		open($chr23, ">>$temp");
#		}
	}
...
#	print $dbg_handle "bp\tREF\tALT\tVTYPE\t@qc_names\n";	
	...
	for($i = 0; $i < @qc_names; $i++){												# setup various headers needed for summary files
		...
		$headers[$i] =~ s/,$//;
		$outnames[$i] = $out_dir . $output_prefix . $qc_names[$i] . $info . "thread_" . $th_name . $parameters{"snv_summary"};
		print reu "$outnames[$i]\n";
		open($outstream[$i],">$outnames[$i]");
		$temp = $outstream[$i];
		if($th_name == 0){																# only thread_0 writes a header line
	...
	$temp = $`;
	$temp = reverse($temp);
	$temp = $scratch_dir . "flagged_" . "thread_" . "$th_name" . "_" . $temp;													# outfile.vcf, either compressed or uncompressed just like input.vcf

#die "temp out is: $temp\n";
	if($printflg){
		if($usegz){
...
# all the individual stats are not used in the multi-allelic case but probably will at some point in the future. So they stay in the code for now.
	...
	$line = $parameters{"chr"} . ":" . $parameters{"start_bp"} . "-" . $parameters{"end_bp"};
print "thread $th_name query($line)\n";
	$iter = $tabix->query($line);
...
=cut
	...
	$tabix->close;
	close(coh);
#	print_indiv_master(\%parameters, \@indivmaster, $const_max_gt, \@keep, \@real_sex, $th_name);						
	print "\n";
	close(dbg);
	if($chr23){
		close($chr23);
	}
	$outfile = $scratch_dir . "indiv_" . "$th_name" . "_.txt";
...
#		}
	...
	close(out);
	if($th_name == 0){
		open(out,">$scratch_dir/keep.txt");
		print out "@keep\n";
		close(out);
		open(out,">$scratch_dir/sex.txt");
		print out "@real_sex\n";
		close(out)
	}
=begin
...
#	my($th_name) = $_[5];
	...
	my($total_gt);
	my($ratio);
	my($outfile) = $parameters{"output_dir"};
	my($missing);
	my($titv);
	my($t1, $t2, $t3, $t4);
	my($passstr, $failstr);
	$outfile .= $parameters{"chr"} . "_";
#	$outfile .=  "thread_" . "$th_name" . "_";
...
=cut
	...
	if($found == 0){
		print STDERR "Did not find threads: default = 1\n";
		$hash{"threads"} = 1;
	}
	...
	while(chomp($line = <inf>)){
		...
		if($line =~ m/^output_prefix:/){
			...
			$temp = $&;
			if($temp =~ m/\//){
				die "output_prefix: must not contain path info like: $temp\n";
			}
	...
	while(chomp($line = <inf>)){
		...
		if($line =~ m/^output_dir:/){
			...
			$hash{"output_dir"} = $temp;
			$found = 1;
			last;
		}
	}
	if($found == 0){
		print STDERR "Did not find output_dir: default = \"./\"\n";
		$hash{"output_dir"} = "./";
	}
	...
	while(chomp($line = <inf>)){
		...
		if($line =~ m/^scratch_dir:/){
			...
			$hash{"scratch_dir"} = $temp;
			$found = 1;
			last;
		}
	}
	if($found == 0){
		print STDERR "Did not find qc_subset_col: default = ./\n";
		$hash{"scratch_dir"} = "./";
	}
...
sub display_parameters
{
	...
	$temp = reverse($temp);
	$temp .= ".txt";
	$temp = $hash{"output_dir"} . "parameters_" . $temp;
	open(out,">$temp") or die "Could not open $temp\n";
...
sub read_pedfile
{
	...
	close($stream);
	$temp = $scratch . "/temp_input.fam";
	fix_infile($infile, $temp);																# this function depends on the .fam file du jour
...
sub cleanup
{
	...
	foreach $file (@files){
		if($file =~ m/^reunion_\d+_snv/){
			unlink($file);
			next;
		}
		if(($file =~ m/^flagged_thread_\d+/) && ($file =~ m/\.gz$/)){
			if(index($file, $chr) != -1){
				unlink($file);
			}
			next;		
		}
		if(($file =~ m/thread_\d+snv/) && ($file =~ m/\.out$/)){
			unlink($file);
			next;		
		}		
	}
	chdir($pwd);
	rmdir($temp) or die "Unable to delete $temp\n";
}
